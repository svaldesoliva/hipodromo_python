...
while True:
    ...
    if cuser > 0 and cuser <= n:
        if apuesta <= dinero and apuesta > 0:
            ...
            if cuser == ganador:
                dinero += (apuesta * 2)
                cprint(f"\nGanas ${apuesta * 2} pesos.\nTu dinero total es: ${dinero}", 'light_green')
                input("Presiona enter para seguir jugando...")
            else:
                cprint(f"Perdiste ${apuesta} pesos.\nTu dinero total es: ${dinero}.", 'light_red')
                input("Ingresa enter para seguir jugando...")
        else:
            input("Error, ingresa un monto válido. Presiona enter para continuar...")
        if dinero == 0:
            input("Te quedaste sin dinero, sal de mi hipódromo pobre de mierda. Presiona enter para salir...")
            break
    else:
